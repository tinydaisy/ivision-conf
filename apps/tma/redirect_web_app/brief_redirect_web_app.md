...
## Карта страниц — где редактировать

При добавлении новой страницы нужно обновить **два файла**:

### 1. `pages.json` — для веб-редиректов (используется в будущем)
...
### 2. `bot.html` — карта страниц в мини-аппе (синхронная, для мгновенного редиректа)
...
> **Почему дублируется?** Telegram Mini App требует мгновенного редиректа. Если загружать `pages.json` через `fetch()` (сетевой запрос), Telegram может показать «не удалось загрузить страницу» до того, как редирект сработает. Поэтому в `bot.html` карта хранится синхронно.
...
## Как добавить новую страницу (4 шага)
...
### Шаг 1б — добавить страницу в `bot.html` (карта PAGES)
